...
class Lobby extends React.Component {
  constructor(props) {
    ...
    this.io = io.connect(
      "ws://localhost" + process.env.NODE_ENV === "development" ? ":4000" : "",
      {
        //Should default to using same port as the express server if used in production. If in dev, use 4000
        forceNew: false,
  ...
  setupListeners() {
    ...
    this.io.on("kick", () => {
      ...
      this.setState({
        error: {
          type: "kicked",
          msg: "You were kicked by the lobby leader. Sorry!",
        },
      });
      setTimeout(() => {
        this.setState({ error: null });
  ...
  componentDidMount() {
    ...
    fetch(`/api/lobbyexists?id=${this.props.match.params.id.toUpperCase()}`)
      .then((res) => {
  ...
  connect() {
    ...
    //TODO: add callback function to catch error
  ...
  render() {
    ...
    if (this.state.lobbyExists === undefined) {
      ...
      if (
        ...
        this.state.lobbyState.inProgress == true
      ) {
        ...
        <div style={{ margin: "10px 10px 10px 10px" }}>
          ...
          {this.state.connected && this.state.lobbyState ? (
            <>
              <p>
                You are{" "}
                {this.getPlayerById(this.state.myId) != null
                  ? this.getPlayerById(this.state.myId).name
                  : "[error]"}
              </p>
              <br />
              <div
                ...
                <p>
                  Max Players: {this.state.lobbyState.maxPlayers}{" "}
                  {this.amLobbyLeader() ? (
                    <span>
                      <button
                        onClick={this.changeLobbyParam.bind(
                          this,
                          "maxPlayers",
                ...
                <p>
                  ...
                  <input
                    ...
                    onChange={this.changeLobbyParam.bind(
                      this,
                      "isPrivate",
              ...
              <h2>Player List:</h2>
              <ul>
                {this.state.lobbyState.players.map((v) => (
                  <li>
                    {v.name + (v.isLeader ? " [LEADER]" : "")}{" "}
                    {this.amLobbyLeader() && !v.isLeader ? (
                      <button
                        onClick={() => {
            ...
            <div>
              <p>
                Username: <input id="username" placeholder={name}></input>
              </p>
